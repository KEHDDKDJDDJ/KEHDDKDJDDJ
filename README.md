local redzlib = loadstring(game:HttpGet("https://raw.githubusercontent.com/Toilaai4141/Cac/main/source.lua%20(5).txt"))()
local Window = redzlib:MakeWindow({ Title = "blackX", SubTitle = "by _black", SaveFolder = "redzHub-MemeSea.json" })
Window:AddMinimizeButton({
  Button = { Image = "rbxassetid://18897840821", BackgroundTransparency = 0 },
  Corner = { CornerRadius = UDim.new(0, 6) }
})
local Tabs = {
  Discord = Window:MakeTab({"Discord", "Info"}),
  MainFarm = Window:MakeTab({"Doors", "eye"}),
  Items = Window:MakeTab({"Car dealership", "Car"}),
  Stats = Window:MakeTab({"Restaurant tycoon 2", "coffee"}),
  Teleport = Window:MakeTab({"Meme sea", "swords"}),
  Shop = Window:MakeTab({"Kat", "Sword"}),
  Misc = Window:MakeTab({"Coming Soon", "warning"})
}
local _Discord = Tabs.Discord do
  _Discord:AddDiscordInvite({
    Name = "blackX",
    Description = "copy for new updates information",
    Logo = "rbxassetid://18685184623",
    Invite = "https://www.youtube.com/@BlackBl0x-u8d"
  })
end

Window:SelectTab(Tabs.Teleport)

local _Teleport = Tabs.Teleport do
  _Teleport:AddDropdown({"Farm Tool", Loaded.WeaponsList, Settings.ToolFarm, function(Value)
    Settings.ToolFarm = Value
  end, "Main/FarmTool"})
  _Teleport:AddSection("Farm")
  AddToggle(_Teleport, {"Auto Farm Level", ("MaxLevel: %i"):format(MSetting.Setting.MaxLevel)}, "Level Farm")
  AddToggle(_Teleport, {"Auto Farm Nearest"}, "Nearest Farm")
  _Teleport:AddSection("Enemies")
  _Teleport:AddDropdown({"Select Enemie", Loaded.EnemeiesList, {Loaded.EnemeiesList[1]}, function(Value)
    _env.SelecetedEnemie = Value
  end, "Main/SEnemy"})
  AddToggle(_Teleport, {"Auto Farm Selected"}, "FS Enemie")
  AddToggle(_Teleport, {"Take Quest [ Enemie Selected ]", true}, "FS Take Quest")
  _Teleport:AddSection("Boss Farm")
  AddToggle(_Teleport, {"Auto Meme Beast [ Spawns every 30 Minutes ]", "Drops: Portal ( <25% ), Meme Cube ( <50% )"}, "Meme Beast")
  _Teleport:AddSection("Raid")
  AddToggle(_Teleport, {"Auto Farm Raid", "Req: Level 1000"}, "Raid Farm")
end
